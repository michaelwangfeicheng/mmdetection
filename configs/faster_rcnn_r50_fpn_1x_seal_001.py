...
dist_params = None #dict(backend='nccl')                # 分布式参数
log_level = 'INFO'                                # 输出信息的完整度级别
# work_dir = './output/seal_detection_train/faster_rcnn_r50_fpn_1x'   # log文件和模型文件存储路径
# load_from = 'checkpoints/faster_rcnn_r50_fpn_1x_20181010-3d1b3351.pth'     # 加载模型的路径，None表示从预训练模型加载
...
data_root = 'data-generator/synthetic-images-master/data/'     # 数据集根目录
img_norm_cfg = dict(
...
data = dict(
    ...
    train=dict(
        ...
        ann_file=data_root + 'annotations/seal_train.json',    # 数据集annotation路径
        img_prefix=data_root + 'train/',     # 数据集的图片路径
        pipeline=train_pipeline),
    val=dict(                                    # val、test 同上
        type=dataset_type,
        ann_file=data_root + 'annotations/seal_train.json',
